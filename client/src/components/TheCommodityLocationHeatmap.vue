...
const { frequencyCommodityLocations } = storeToRefs(store)
...
function drawHeatmap(heatmapData) {
  ...
  //   .enter()
  //   .append('text')
  //   .attr('x', d => x(d.commodity_id) + x.bandwidth() / 2)
...
watch([frequencyCommodityLocations], ([newFrequencyCommodityLocations]) => {
  if (newFrequencyCommodityLocations.length > 0)
    drawHeatmap(newFrequencyCommodityLocations)
}, { immediate: true })
</script>
