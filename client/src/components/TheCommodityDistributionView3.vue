...
function transformData(data: any): any {
  ...
  data.forEach(item => {
    const existing = result.find(
      res => (res.date === item.date && res.location_id === item.location_id)
    )

    if (existing) {
      existing.total_records += 1
      existing.total_tons += item.qty_tons > 0 ? item.qty_tons : 0
    }
    else {
      result.push({
        date: item.date,
        ...
        total_records: 1,
        total_tons: item.qty_tons > 0 ? item.qty_tons : 0,
      })
    }
  })
...
function renderChart(commodityDistributions: any, selectedLocationIDs: any, selectedCommodityIDs: any, dateInterval: any): void {
  ...
  d3.select('#commodity3').selectAll('*').remove()
  const svg = d3.select('#commodity3')
    .append('svg')
    .attr('width', width)
    .attr('height', height)
    .append('g')
  ...
  const yLocation = d3.scaleBand()
    .domain(combinedData.map((d: any) => d.location_id))
    .range([0, _height - 10])
    .padding(0.1)
  ...
  const addition = 0
  const colorAggregated = d3.scaleLinear()
    .domain([d3.min(combinedData, d => d.total_tons / d.total_records), d3.max(combinedData, d => d.total_tons / d.total_records) || 10])
  ...
  svg.selectAll('.vessel')
    ...
    .attr('y', (d: any) => yLocation(d.location_id) as number)
    .attr('width', barWidth) // 占至少一天的长度
    .attr('height', yLocation.bandwidth())
    .attr('fill', (d: any) => colorAggregated(d.total_tons / d.total_records))
    .attr('stroke', 'none')
    .on('mouseover', function (event, d: any) {
      tooltip.style('display', 'block')
        .html(`Date: ${d3.timeFormat('%Y-%m-%d')(d.date as Date)}<br>Location ID: ${d.location_id}<br>Total Tons:${d.total_tons}`)
  ...
  legend.append('g')
    .selectAll('rect')
    .data(d3.range(d3.min(combinedData, d => d.total_tons / d.total_records), d3.max(combinedData, d => d.total_tons / d.total_records) || 10, 1))
    .enter().append('rect')
    .attr('y', (d, i) => legendScale(d))
    .attr('width', 20)
    .attr('height', legendScale(1) - legendScale(0))
    .attr('fill', (d) => colorAggregated(d))
}
