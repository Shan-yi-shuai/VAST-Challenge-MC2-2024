...
function renderChart(commodityDistributions: any, selectedLocationIDs: any, selectedCommodityIDs: any, dateInterval: any): void {
  ...
  d3.select('#commodity4').selectAll('*').remove()
  const svg = d3.select('#commodity4')
    .append('svg')
    .attr('width', width)
    .attr('height', height)
    .append('g')
  ...
  svg.selectAll('.vessel')
    ...
    .on('mouseover', function (event, d: any) {
      tooltip.style('display', 'block')
        .html(`Date: ${d3.timeFormat('%Y-%m-%d')(d.date as Date)}<br>Commodity ID: ${d.commodity_id}<br>Total Tons:${d.total_tons}`)
