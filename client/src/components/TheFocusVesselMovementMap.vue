...
const { vesselMovements, locationCoordinates, geoData, focusVesselID, selectedLocationIDs, dateInterval, locationID2Location } = storeToRefs(store)
...
watch([vesselMovements, focusVesselID, selectedLocationIDs, dateInterval], ([newVesselMovements, newFocusVesselID, newSelectedLocationIDs, newDateInterval]) => {
  if (newVesselMovements.length > 0
    ...
    drawMovements(geoData.value, locationCoordinates.value, newVesselMovements, [newFocusVesselID], newSelectedLocationIDs, newDateInterval)
}, { immediate: true })
</script>
