...
function dwellVesselLocations(vesselMovements) {
  const dwellDict: any = {}
  // 计算每个船只在每个地点出现的次数
  for (const movement of vesselMovements) {
    const { vessel_id, location_id } = movement
    if (!dwellDict[vessel_id])
      dwellDict[vessel_id] = {}
    if (!dwellDict[vessel_id][location_id])
      dwellDict[vessel_id][location_id] = 0
    // Convert the start and end times to Date objects
    const startTime = new Date(movement.start_time)
    const endTime = new Date(movement.end_time)
    const durationInMilliseconds = endTime.getTime() - startTime.getTime()
    dwellDict[vessel_id][location_id] += durationInMilliseconds / 1000
  }
  // 转换为所需的数组格式
  const dwellArray = []
  for (const vessel_id in dwellDict) {
    for (const location_id in dwellDict[vessel_id]) {
      dwellArray.push({
        vessel_id,
        location_id,
        dwell: dwellDict[vessel_id][location_id],
      })
    }
  }
...
function drawHeatmap(vesselMovements, selectedVesselIDs, selectedLocationIDs, dateInterval) {
  ...
  const heatmapData = dwellVesselLocations(filterVesselMovements)

  const margin = { top: 5, right: 5, bottom: 5, left: 5 }
  // 设置 SVG 画布
  const _width = width - margin.left - margin.right
  const _height = height - margin.top - margin.bottom
  d3.select('#heatmap2').selectAll('*').remove()
  ...
  const colorScale = d3.scaleSequential(d3.interpolateBlues)
    .domain([0, d3.max(heatmapData, d => d.dwell)])
  ...
  svg.selectAll()
    ...
    .style('fill', d => colorScale(d.dwell))
    .on('mouseover', function (event, d: any) {
      tooltip.style('display', 'block')
        .html(`Vessel ID: ${d.vessel_id}<br>Location ID: ${d.location_id}<br>Total Dwell: ${d.dwell}`)
