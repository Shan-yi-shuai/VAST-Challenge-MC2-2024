...
function transformData(data: any): any {
  ...
  data.forEach(item => {
    ...
    if (existing) {
      existing.commoditys.push(item.commodity_id)
    }
    else {
      result.push({
        date: item.date,
        location_id: item.location_id,
        commoditys: [item.commodity_id],
      })
    }
  })
...
function renderChart(commodityDistributions: any, selectedLocationIDs: any, selectedCommodityIDs: any, dateInterval: any): void {
  ...
  d3.select('#commodity').selectAll('*').remove()
  const svg = d3.select('#commodity')
    .append('svg')
    .attr('width', width)
    .attr('height', height)
    .append('g')
  ...
  const colorAggregated = d3.scaleLog()
    .domain([1, d3.max(combinedData, d => d.commoditys.length) + addition || 10])
  ...
  svg.selectAll('.vessel')
    ...
    .attr('fill', (d: any) => colorAggregated(d.commoditys.length + addition))
    .attr('stroke', 'none')
    .on('mouseover', function (event, d: any) {
      tooltip.style('display', 'block')
        .html(`Date: ${d3.timeFormat('%Y-%m-%d')(d.date as Date)}<br>Location ID: ${d.location_id}<br>Commodity Num:${d.commoditys.lenght}`)
  ...
  legend.append('g')
    .selectAll('rect')
    .data(d3.range(1, d3.max(combinedData, d => d.commoditys.length) + addition || 10, 1))
    .enter().append('rect')
