<template>
    <el-select v-model="value" multiple clearable collapse-tags placeholder="Select" popper-class="custom-header"
        ...
        <el-option v-for="item in filteredData" :key="item.id" :label="item.label" :value="item.id" />
    </el-select>
</template>
...
const { commodities, selectedCommodityIDs } = storeToRefs(store)
const data: Ref<any[]> = commodities

const searchQuery = ref<string>('')

const value = computed({
    get: () => selectedCommodityIDs.value,
    set: (val: CheckboxValueType[]) => {
        selectedCommodityIDs.value = val
    }
})
