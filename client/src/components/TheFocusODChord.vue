...
const { vesselMovements, focusVesselID, selectedLocationIDs, dateInterval, locationColor, locationID2Name } = storeToRefs(store)
...
function drawChordDiagram(vesselMovements: any[], focusVesselID: any, selectedLocationIDs: any[], dateInterval: any[]) {
  const filterVesselMovements = vesselMovements.filter((d: any) => {
    return focusVesselID === d.vessel_id
  ...
  d3.select('#chord2').selectAll('*').remove()
  const svg = d3.select('#chord2')
    .append('svg')
    .attr('width', width)
    .attr('height', height)
    .append('g')
...
watch([vesselMovements, focusVesselID, selectedLocationIDs, dateInterval], ([newVesselMovements, newFocusVesselID, newSelectedLocationIDs, newDateInterval]) => {
  if (newVesselMovements.length > 0
    ...
    drawChordDiagram(newVesselMovements, newFocusVesselID, newSelectedLocationIDs, newDateInterval)
}, { immediate: true })
</script>
